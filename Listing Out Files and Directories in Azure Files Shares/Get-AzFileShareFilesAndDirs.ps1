...
function list_subdir([Microsoft.WindowsAzure.Commands.Common.Storage.ResourceModel.AzureStorageFileDirectory]$dirs) {
    ...
    foreach ($f in $filesAndDirs) {
        ...
        if ($f.gettype().name -eq "AzureStorageFile") {
            ...
            $fileSmbLastWriteTime = $($f.FileProperties.SmbProperties.FileLastWrittenOn.DateTime).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
            ...
            Write-Host "SMB Last Write Time: $fileSmbLastWriteTime" -ForegroundColor Blue
        ...
        elseif ($f.gettype().name -eq "AzureStorageFileDirectory") {
            ...
            $dirSmbLastWriteTime = $($f.ShareDirectoryProperties.SmbProperties.FileLastWrittenOn).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffffffK")
            ...
            Write-Host "SMB Last Write Time:" $dirSmbLastWriteTime -ForegroundColor Red
            Write-Output ""
